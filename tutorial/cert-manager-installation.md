...
Note: cert-manager uses Kubernetes Lease objects for leader election, which is enabled by default. These leases are typically created in the kube-system namespace, but the namespace can be configured. They are not automatically removed when uninstalling cert-manager. You can safely delete them manually:
